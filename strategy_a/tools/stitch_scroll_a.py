...
def open_rgb_pipe(
    ...
    server-side before piping; combined with pix_fmt="gray" this gave 3.11x
    on lexiconv.mp4 in bench_ffmpeg_pipes.py option 4. Caller must size
    its read buffer / reshape to (h, w).
    """
    cmd = [ffmpeg, "-v", "error"]
    if hwaccel:
        cmd += ["-hwaccel", hwaccel]
    ...
    # read; bufsize ~16 MB -> 20 s. See AGENTS.md "Performance".
...
def match_1d_offset(
    ...
    Implementation: numeric core is a Numba @njit kernel. See sibling
    stitch_scroll_b.py for the detailed history and the AGENTS.md
    "Performance / hardware acceleration" section.
    """
    if ref.ndim == 1:
